...
public class CourseRepo implements ICourseRepo {
    ...
    public Course save(String name, LocalDateTime date, int duration, float price, CourseCategory category, Teacher teacher, ExperienceLevel experienceLevel, int maxParticipants) {
        ...
        Course course = null;
        course = new Course(name, date, duration, price, category, teacher, experienceLevel, maxParticipants);
        if(findOne(course.getId()).isPresent()) {
            return update(course, name, date, duration, price, category, experienceLevel, maxParticipants);
        }
        else {
            try (PreparedStatement prepStmt = conn.prepareStatement("insert into course (name, date, duration, price, category, idTeacher, experiencelevel, maxparticipants) values (?, ?, ?, ?, ?, ?, ?, ?)")) {
            ...
            return new Course(name, date, duration, price, category, teacher, experienceLevel, maxParticipants);
        }
    }
    ...
    public Course update(Course course, String name, LocalDateTime time, int duration, float price, CourseCategory category, ExperienceLevel experienceLevel, int maxParticipants) {
        Connection conn = dbUtils.getConnection();
        try (PreparedStatement prepStmt = conn.prepareStatement("update course set name = ?, date = ?, duration = ?, price = ?, category = ?, experiencelevel = ?, maxparticipants = ? where id = ?")) {
        ...
        return new Course(course.getId(), course.getName(), time, duration, price, category, course.getTeacher(), experienceLevel, maxParticipants);
    }
